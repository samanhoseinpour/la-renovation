...
`/admin` is a private, noindexed area for the two principals and the site administrator. It opens on an overview: three stat tiles backed by real counts (new enquiries, the last seven days, failed notification emails), the newest submissions, and each admin's last recorded sign-in. The submissions inbox searches across name, email, company and message text, pages on a keyset cursor instead of capping the list, and handles bulk work through row checkboxes with select-all: mark read or unread, archive, delete behind a confirm, or export to CSV. Exports cover either the checked rows or the current view over office-local date ranges (today, last 7 or 30 days, this month, all time), with spreadsheet formula injection neutralized. A submission whose notification email failed carries a resend action that records an honest new failure reason if the retry fails too. Settings adds a profile header, the passkey manager, the password form, and an active-sessions card with per-session revoke and a sign-out-everywhere-else button; sessions show timestamps only, because device details are deliberately never stored.
