...
- `/contact` — a sticky intro rail (title, response promise, address, hours, phone, email) beside the enquiry form, whose boxed fields match the admin sign-in's: placeholders in every text field, an asterisk on the two required ones, error lines that never shift the layout, and a full-width send button. The fields stay grouped under visible labels ("About you" / "The project"): tap-to-toggle chips pick any number of project types (a division page's closing CTA arrives with its own type already checked), radio chips the stage, and the message is optional; the browser pre-checks the same zod schema the server enforces, so errors show inline on submit and clear per field as they're fixed. An unsent form keeps a draft in the tab's session storage, so a mid-enquiry detour costs nothing, and the rail closes on a pointer to `/faq` for visitors who came with a question rather than a project. The confirmation panel offers to send another enquiry without a reload. Backed by a server action that stores the enquiry, then notifies the office by email (see Contact form delivery)
...
The root layout mounts Vercel Web Analytics — cookieless, aggregate page counts with nothing stored on the visitor's device. It's enabled per-project in the Vercel dashboard and no-ops locally. One custom event rides the same pipeline: a bare "Enquiry submitted" count with no properties, fired when the contact form's action reports success — trapped bot submissions look successful by design, so they count too. Vercel records custom events on Pro and above, so on the current Hobby tier the event fires but stays invisible until an upgrade; until then, conversion reads as `/contact` page views against the admin overview's submission counts. Speed Insights was deliberately not purchased (it's a paid add-on; Search Console's Core Web Vitals report covers field data for free). `/privacy` describes exactly what measurement collects, so that page must move in the same commit as any measurement change.
